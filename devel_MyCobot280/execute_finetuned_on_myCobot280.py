...
flags.DEFINE_string("port_camera", "/dev/video1", "Port of the camera")
flags.DEFINE_spaceseplist("goal_eep", [-4.74, 18.54, -11.25, -77.6, 11.33, -63.45], "Goal position")
...
def main(_):
    ...
    def sample_actions(
        ...
        observations = jax.tree_map(lambda x: x, observations)
    ...
    while True:
        ...
        while t < FLAGS.num_timesteps:
            if time.time() > last_tstep + STEP_DURATION:
                ...
                action = np.array(policy_fn(frame, task), dtype=np.float64)
